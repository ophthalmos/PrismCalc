...
namespace PrismCalc
{
    public partial class Form1 : Form
    {
        ...
        private readonly double maxPwr = 50;                   // maximale Prismenstärke
        private bool reyHorizont, reyVertical;
        private double powerHorizont, powerVertical, basisHorizont, basisVertical;
        private double reyPower, leyPower, resPower, reyBase, leyBase;
        private readonly double pt = 19.0;                     // prismthickness = Maßfaktor für die Grafiken
        private readonly int x0RE = 50;
        private readonly int y0RE = 115;
        private readonly int x0LE = 144;
        private readonly int y0LE = 115;
        private readonly int x1 = 205;
        private readonly int y1 = 184;
        private readonly int nd = 36;
        private readonly int d5 = 180;
        private string cbTxt;                         // Clipboard-Text, wird beim Beenden erstellt
        ...
        {
            double.TryParse(nudOblBas.Text, out var axs);
        ...
        {
            {
                try { System.Diagnostics.Process.Start("https://www.schielen.de/software.htm"); }
        ...
        private void CalcDivide()
        {
            ...
            double.TryParse(nudOblPwr.Text, out var powerObl);
            if (powerObl > maxPwr) { powerObl = maxPwr; nudOblPwr.Text = powerObl.ToString("0.00"); }
            double.TryParse(nudOblBas.Text, out var basisObl);
            if (basisObl > 360) { basisObl = 360; nudOblBas.Text = basisObl.ToString("0"); }
            var rhpPwr = Math.Round(Math.Cos(Deg2Rad(basisObl)) * powerObl, 2);
            var rvpPwr = Math.Round(Math.Sin(Deg2Rad(basisObl)) * powerObl, 2);
            //var rhpBas = Math.Round(90 - (rhpPwr / rhpPwr * 90));
            //var rvpBas = Math.Round(180 - (rvpPwr / rvpPwr * 90));
            var rhpBas = rhpPwr < 0 ? 180 : 0;
            var rvpBas = rvpPwr < 0 ? 270 : 90;
            ...
            using (Graphics g = Graphics.FromImage(bm)) // Compiler baut Dispose ein
            {
                ...
                var sansFont = new Font("MS Sans Serif", 10);
                ...
                if (rvpPwr >= 0)
                {
                    var startAngle = 0.0F; // 180.0F;
                    var sweepAngle = 360F; // 180.0F;
                    for (var i = nm; i >= 1; i--) // Anzahl der Halbkreise, Bogen nach oben
                    {
                ...
                {
                    var startAngle = 0.0F;
                    var sweepAngle = 360.0F; // 180.0F;
                    for (var i = nm; i >= 1; i--) // Anzahl der Halbkreise, Bogen nach unten
                    {
        ...
        {
            if (tabControl1.SelectedIndex == 0) { nudPowerHorizont.Focus(); }
            else { nudOblPwr.Focus(); }
        }
        ...
        private void NudOblBas_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            else if (axs == 0 && e.KeyCode == Keys.Down) { axs = 360; nudOblBas.Text = axs.ToString(); }
            else if (axs == 360 && e.KeyCode == Keys.Up) { axs = 0; nudOblBas.Text = axs.ToString(); }
        }
        ...
        public static double Rad2Deg(double radians) => (180 / Math.PI) * radians;

        private void Conversion()
        { // C# rechnet in Radians (nicht Grad) => jeweils Umrechnung erforderlich.
            var FromDegrees = lblConvertFrom.Text.Contains("degree");
        ...
        {
            var foo = lblConvertFrom.Text;
            var bar = lblConvertTo.Text;
        ...
        public void DrawComplete()
        {
            ...
            var bm = new Bitmap(wid, hgt); // using führt zu Absturz
            using (var g = Graphics.FromImage(bm)) // Compiler sorgt für Dispose
            {
                DrawDummy(g); // enthält Befehl zum Löschen der Grafiken
                // EINGABE      
                var pt2 = powerHorizont > 0 && powerVertical > 0 ? pt * 1.5 : pt;
                ...
                // AUSGABE
                var ptx = resPower > 0 ? resPower / (powerHorizont + powerVertical) : 1;
                ...
                var cbTxR = reyPower > 0 ? "RA " + reyPower.ToString("0.00") + " cm/m Base " + reyBase.ToString("0") + "°" : "";
                var cbTxL = leyPower > 0 ? "LA " + leyPower.ToString("0.00") + " cm/m Base " + leyBase.ToString("0") + "°" : "";
        ...
        {
            var err = false; int x0, y0;
            if (re) { x0 = x0RE; y0 = y0RE; } else { x0 = x0LE; y0 = y0LE; }
            var rPoint = new PointF(x0, y0); Matrix mtrx = new Matrix();
            ...
            else
            {
                ...
                {
                    var p = new Pen(Color.Red); g.DrawPolygon(p, points);
                    p.DashStyle = DashStyle.Dot; g.DrawLine(p, x0 + 30, y0, x0 + 45, y0);
                }
                else if (art == 4) // Ergebnis LINKES Auge
                {
                    var p = new Pen(Color.Blue); g.DrawPolygon(p, points);
                    p.DashStyle = DashStyle.Dot; g.DrawLine(p, x0 + 30, y0, x0 + 45, y0);
                }
            }
        }
        ...
        private void Calculation() // Berechnung bei schräger Prismenbasislage
        {
            ...
            reyHorizont = rbPrsmHorizontRE.Checked;
            reyVertical = rbPrsmVerticalRE.Checked;
        ...
        private void PrismCalcForm_Load(object sender, EventArgs e)
        {
            CalcDivide();
            DrawComplete(); // Startgrafiken zeichenen
        }
        ...
        {
            {
                try { System.Diagnostics.Process.Start("http://www.schielen.de/"); }
                catch (Exception ex) { MessageBox.Show(ex.Message, "Fehler", MessageBoxButtons.OK, MessageBoxIcon.Error); }
            }
        }

        private void BtnF1_Click(object sender, EventArgs e) => f2?.ShowDialog(); // F1 würde mehrfach Form öffnen
        ...
        private void BtnF2_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else { MessageBox.Show("Es liegt keine Berechnung vor!", "Prismenrechner", MessageBoxButtons.OK, MessageBoxIcon.Error); }
            }
